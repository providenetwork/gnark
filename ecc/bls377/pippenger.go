...
func (p *G1Jac) Gus549(curve *Curve, points []G1Affine, scalars []fr.Element, c int) *G1Jac {
	...
	buckets := make([]G1Jac, (1<<c)-1)
	bucketsInitialized := make([]bool, (1<<c)-1)
	...
	for j := t - 1; j >= 0; j-- {
		...
		for i := 0; i < len(points); i++ {
			...
			if selector != 0 {
				// if this is the first point in the bucket then don't add, use Set instead
				if !bucketsInitialized[selector-1] {
					bucketsInitialized[selector-1] = true
					points[i].ToJacobian(&buckets[selector-1])
				} else {
					buckets[selector-1].AddMixed(&points[i])
				}
			}
